...
export class ActividadesPmGestionComponent implements OnInit {
  ...
  private buildGeneralRows(data: any): RowInput[] {
    ...
    const extras = [
      ['Institución visitada', p?.feriaInstitucionVisitada ?? data.base?.institucionVisitada],
      ['Estudiante feria RUT', p?.feriaEstRut ?? data.base?.feriaEstRut],
      ['Estudiante feria nombre', p?.feriaEstNombre ?? data.base?.feriaEstNombre],
      ['Tema central', p?.jornadaTemaCentral ?? data.base?.temaCentral],
      ['Talleres', p?.jornadaTalleres ?? data.base?.talleres],
      ['Responsable taller', p?.jornadaResponsableTaller ?? data.base?.responsableTaller],
      ['Asignatura', p?.tallerAsignatura ?? data.base?.asignaturaRemedial],
      ['Competencia', p?.tallerCompetencia ?? data.base?.competenciaAReforzar],
      ['Estudiantes beneficiados', p?.tallerNombreEstudiantesBeneficiados ?? data.base?.numeroEstudiantesBeneficiados],
      ['Evento', p?.congresoNombreEvento ?? data.base?.nombreEvento],
      ['Ponencia', p?.congresoPonenciaPresentada ?? data.base?.ponenciaPresentada],
      ['Relator', p?.congresoRelator ?? data.base?.relator],
      ['Colegio asociado', p?.alternanciaColegioAsociado ?? data.base?.colegioAsociado],
      ['Docente colaborador', p?.alternanciaDocenteColaborador ?? data.base?.docenteColaborador],
      ['Asignatura alternancia', p?.alternanciaAsignatura ?? data.base?.asignaturaAlternancia],
      ['Curso', p?.alternanciaCurso ?? data.base?.curso],
      ['Docente asignatura', p?.alternanciaDocenteAsignatura ?? data.base?.docenteAsignatura],
      ['Estudiantes participantes', p?.alternanciaEstudiantesParticipantes ?? data.base?.alternanciaEstudiantesParticipantes],
      ['Nombre actividad', p?.alternanciaNombreActividad ?? data.base?.nombreActividadAlternancia],
      ['Objetivo pedagógico', p?.salidaObjetivoPedagogico ?? data.base?.objetivoPedagogico],
      ['Asignatura vinculada', p?.salidaAsignaturaVinculada ?? data.base?.asignaturaVinculada],
      ['Profesor responsable', p?.salidaProfesorResponsable ?? data.base?.profesorResponsable],
      ['Estudiante salida RUT', p?.salidaEstRut ?? data.base?.salidaEstRut],
      ['Estudiante salida nombre', p?.salidaEstNombre ?? data.base?.salidaEstNombre],
    ];
