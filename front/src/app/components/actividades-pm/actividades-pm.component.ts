    ...
    this.actividadesPmService.crear(request).subscribe({
      next: (res) => {
        ...
        if (!Number.isFinite(id) || id <= 0) {
          this.openDialog(false, 'No se pudo registrar la actividad de vinculación. Intenta nuevamente.');
          return;
        }
        ...
        this.openDialog(true, `Actividad de vinculación registrada exitosamente.${medios}`);
        this.limpiar();
      },
      error: (err) => {
        console.error('Error guardando actividad', err);
        const apiMessage = err?.error?.message || err?.message;
        const fallback = 'No se pudo registrar la actividad de vinculación. Intenta nuevamente.';
